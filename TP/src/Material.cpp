...
std::shared_ptr<Material> Material::empty_material() {
    ...
    if(!material) {
        ...
        material->_program = Program::from_files("g-buffer.frag", "basic.vert");
...
Material Material::textured_material() {
    Material material;
    material._program = Program::from_files("g-buffer.frag", "basic.vert", {"TEXTURED"});
    return material;
}

Material Material::textured_normal_mapped_material() {
    Material material;
    material._program = Program::from_files("g-buffer.frag", "basic.vert", std::array<std::string, 2>{"TEXTURED", "NORMAL_MAPPED"});
    return material;
}
